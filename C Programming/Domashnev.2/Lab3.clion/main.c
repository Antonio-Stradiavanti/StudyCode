...
Задачи:
    - [ ] Посмотреть видео про списки
    - [ ] Выполнить отладку ф-ций добавления в начало, конец, середину списока
    - [ ] Выполнить отладку ф-ции вывода списка, исправить нумерацию блоков
    - [ ] Выполнить отладку ф-ции удаления элемента
    - [ ] Протестировать с большим количеством элементов
Статус:
    30 apr
...
void menu(BlockList* List)
{
    ...
    switch (op)
    {
        case 1:
            newData = GenNewData();
            insToEnd(List, newData);
            printf("newData is inserted to the t of the List\n");
            break;
        case 2:
            newData = GenNewData();
            insToBg(List, newData);
            printf("newData is inserted to the h of the List\n");
            break;
        case 3:
            printf("Enter index : ");
            fflush(stdin);
        ...
        case 7:
            ...
            break;
        case 8:
            printf("[menu]: Let's create multi-aspect search argument\n");
            sArg = GenNewData();
